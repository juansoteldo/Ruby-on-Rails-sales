...
task :send_reminders => :environment do
  ...
  boxes = Streak::Box.all(ENV['PIPE_ID']).select { |b| b.stage_key == quoted_stage and !b.last_email_from.include? 'customtattoodesign.ca' and b.total_number_of_emails == 1 }
  boxes.each do |box|
  	time_sent = Time.at(box.last_email_received_timestamp.to_i/1000).to_date
  	first = 1.day.ago.to_date
  	second = 2.days.ago.to_date
