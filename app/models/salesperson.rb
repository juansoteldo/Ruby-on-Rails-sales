...
def self.all_with_shopify_orders_by_email(params)
    ...
    orders = orders.select do |order|
      order.line_items.any?{|li| li.title.include? 'Deposit' } and User.where(email: order.customer.email).joins(:requests).where(requests: {created_at: "Tue, 7 Jun 2016".to_date..Time.now}).any?
    end
    orders.map {|order|
      order.sales_id = ""
